...
export const askDetailedAssistant = async (question: string, contextData: string): Promise<string> => {
  ...
  try {
    ...
    // Correctly calling generateContent with model and parameters.
...
export const getQuickAdvice = async (prompt: string): Promise<string> => {
  ...
  try {
    const response = await ai.models.generateContent({
      // Use 'gemini-flash-lite-latest' for the lite version per guidelines.
...
export const analyzeReceipt = async (base64Data: string, mimeType: string): Promise<any> => {
  ...
  try {
    const response = await ai.models.generateContent({
      model: 'gemini-3-pro-preview',
      // For multimodal input, contents should be an object containing a 'parts' array.
      contents: {
        parts: [
    ...
    // Access response text directly and parse JSON.
